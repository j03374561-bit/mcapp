...
import { ChevronRight, ChevronLeft, CheckCircle2, XCircle, AlertCircle, Download, RotateCcw, Mail, FileSpreadsheet, FileText } from 'lucide-react';
import { fetchQuestionsForExam } from '../../lib/questionManager';
import { saveResult, exportSingleResultToExcel } from '../../lib/resultsManager';
...
export function QuestionView({ exam, onBack, userName }) {
    ...
    const handleDownloadResults = () => {
        ...
        questions.forEach((q, index) => {
            ...
            const aText = answer.selectedText ? ` (${answer.selectedText})` : '';
            const cText = answer.correctText ? ` (${answer.correctText})` : '';

            content += `| ${index + 1} | ${safeText} | **${answer.selected.toUpperCase()}${aText}** | **${q.correctAnswer.toUpperCase()}${cText}** | ${resultIcon} |\n`;
        });
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="text-sm text-slate-400">
                                                Your answer: <span className={answer.isCorrect ? "text-emerald-400" : "text-red-400"}>
                                                    {answer.selected.toUpperCase()} {answer.selectedText ? `(${answer.selectedText})` : ''}
                                                </span>
                                                {!answer.isCorrect && (
                                                    <span className="ml-2">
                                                        • Correct: <span className="text-emerald-400">{q.correctAnswer.toUpperCase()} {answer.correctText ? `(${answer.correctText})` : ''}</span>
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            <Button onClick={() => {
                                ...
                                exportSingleResultToExcel({
                                    ...
                                    totalQuestions: total,
                                    percentage,
                                    details: answers // Pass detailed answers
                                });
                            }}>
