...
// Fetch all exams (merged mock + local storage)
export const fetchExams = async () => {
    console.log("Fetching exams (Local Mode)...");
    try {
        // Get custom exams from localStorage
        const storedExams = localStorage.getItem('custom_exams');
        const customExams = storedExams ? JSON.parse(storedExams) : [];
        ...
        // Merge
        const mergedExams = [...validMockExams, ...customExams];
...
export const fetchQuestionsForExam = async (examId) => {
    ...
    // Check local storage for custom exams
    try {
        const storedExams = localStorage.getItem('custom_exams');
        if (storedExams) {
            const customExams = JSON.parse(storedExams);
...
export const saveExam = async (examId, examData) => {
    console.log(`Saving exam ${examId} to LocalStorage...`);
    try {
        const storedExams = localStorage.getItem('custom_exams');
        let customExams = storedExams ? JSON.parse(storedExams) : [];

        // Check if update or new
        const index = customExams.findIndex(e => e.id === examId);
        if (index >= 0) {
            customExams[index] = examData;
        } else {
            customExams.push(examData);
        }
...
export const deleteExam = async (examId) => {
    ...
    try {
        const storedExams = localStorage.getItem('custom_exams');
        if (storedExams) {
            let customExams = JSON.parse(storedExams);
